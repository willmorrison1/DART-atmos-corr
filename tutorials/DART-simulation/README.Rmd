...
```{r}
SRF_raw <- data.frame("lambda" = seq(5, 20, by = 1e-5), "value" = seq(0.1, 1, length.out = length(seq(5, 20, by = 1e-5))) * 0.1)
```
...
```{r}
SRFinterp <- function(SRF_raw, simData) {
...
#hereiam - running this
dataDF_out <- dataDF %>%
    dplyr::group_by(x, y, iter, imgType, imageNo, VZ, VA) %>%
    dplyr::summarise(bandValue = sum(integral))
