...
2. [**Model world**](tutorials/Model-world). 
Configure a three-dimensional model world to faciliate the correction.
3. [**Create multi line of sight image**](tutorials/Multi-line-of-sight-images). 
...
5. [**DART image post-processing**](tutorials/DART-simulation-post-processing). 
...
7. [**Image analysis**](tutorials/Image-analysis). Perform some simple analysis on processed images.
...
rmdFiles_base <- list.files(pattern = ".Rmd", recursive = FALSE, full.names = TRUE)
rmdFiles <- rmdFiles[!grepl(rmdFiles_base, rmdFiles)]
baseDir <- getwd()
for (i in 1:length(rmdFiles)) {
  # setwd(file.path(baseDir, dirname(rmdFiles[i])))
  knitr::knit(input = file.path(baseDir, rmdFiles[i]), output = file.path(baseDir, dirname(rmdFiles[i]), "README.md"))
}
# setwd(baseDir)
```